...
async function getCode(browser, phoneNumber, page, x, m, y, mo, d, p, ws) {
    clearTimeout(timer);
    const {getRandom} = require('./register');
    axios.get("http://www.mili18.com:9180/service.asmx/GetYzm2Str?token=" + token + "&xmid=" + itemID + "&hm=" + phoneNumber + "&sf=1").then(async result => {
        ...
        if (authCode.indexOf('NIKE') !== -1) {
            await page.type('input.code', authCode.substr(authCode.length - 6, authCode.length), { delay: getRandom(2000, 100) });
            ...
            await page.type('input[name="lastName"]', x, { delay: getRandom(2000, 100) });
            await page.waitFor('input[name="firstName"]');
            await page.type('input[name="firstName"]', m, { delay: getRandom(2000, 100) });
            await page.waitFor('input[name="dateOfBirth"]');
            const dateOfBirth = await page.$('input[name="dateOfBirth"]');
            await dateOfBirth.click();
            await page.type('input[name="dateOfBirth"]', y.toString(), { delay: getRandom(2000, 100) });
            // await page.keyboard.down('Tab');
            await page.waitFor(500);
            await page.type('input[name="dateOfBirth"]', mo.toString(), { delay: getRandom(2000, 100) });
            await page.waitFor(500);
            await page.type('input[name="dateOfBirth"]', d.toString(), { delay: getRandom(2000, 100) });
            await page.waitFor('input[name="password"]');
            await page.type('input[name="password"]', p, { delay: getRandom(2000, 100) });
